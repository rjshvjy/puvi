...
COMPLETE REPLACEMENT - Fixed variety code extraction issue
Version: 2.4 - FIXED: Handles all traceable code formats properly
"""
...
def create_sku_production():
    ...
    try:
        ...
        cur.execute("""
            SELECT COALESCE(MAX(
                CAST(SUBSTRING(traceable_code FROM '[0-9]+
...
def allocate_oil_for_production():
    ...
    try:
        ...
        close_connection(conn, cur)) AS INTEGER)
            ), 0) + 1
            FROM sku_production
