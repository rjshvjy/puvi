...
const Purchase = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (mastersAPI?.getCategories) {
        console.log('Using mastersAPI.getCategories');
        data = await mastersAPI.getCategories();
  ...
  const fetchSubcategories = async (categoryId) => {
    try {
      ...
      if (mastersAPI?.getSubcategories) {
        console.log('Using mastersAPI.getSubcategories');
        data = await mastersAPI.getSubcategories(categoryId);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      } else if (api?.purchase?.getSuppliers) {
  ...
  const fetchPurchaseHistory = async () => {
    try {
      ...
      // Try using purchaseAPI
      if (purchaseAPI?.getHistory) {
        response = await purchaseAPI.getHistory({ limit: 20 });
      } else if (api?.purchase?.getHistory) {
        response = await api.purchase.getHistory({ limit: 20 });
      } else {
        // Direct fetch as fallback
  ...
  const handleCreateMaterial = async (e) => {
    ...
    try {
      ...
      // Use purchaseAPI to create material
      if (purchaseAPI?.createMaterial) {
        data = await purchaseAPI.createMaterial(newMaterial);
